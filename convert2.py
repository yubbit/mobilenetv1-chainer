...
### TEST ROUTINE ###
    node_tf = tf.get_default_graph().get_tensor_by_name('MobilenetV1/MobilenetV1/Conv2d_13_pointwise/Relu:0')
    val_tf = sess.run(node_tf, feed_dict={x:img_tf})
    val_tf = sess.run(model_tf, feed_dict={x:img_tf})

with chainer.using_config('train', False):
    model_ch.pick = 'conv2d_13'
    model_ch.pick = 'prob'
    val_ch = model_ch(img_ch).data

print(np.argmax(val_tf, axis=1))
print(np.argmax(val_ch, axis=1))
...
### SAVE ROUTINE ###
chainer.serializers.save_npz('mobilenet_v1_1.0_224.npz', model_ch)
